...
deploy :; forge script script/DeployMinimal.s.sol ${NETWORK_ARGS}
...
sendUserOp :; forge script script/SendPackedUserOp.s.sol ${NETWORK_ARGS}

NETWORK_ARGS := --rpc-url http://localhost:8545 --private-key $(DEFAULT_ANVIL_KEY) --broadcast

ifeq ($(findstring --network arb_sepolia,$(ARGS)),--network arb_sepolia)
	NETWORK_ARGS := --rpc-url $(ARBITRUM_SEPOLIA_RPC_URL) --account $(TESTNET_ACCOUNT) --sender $(SENDER) --broadcast --verify --etherscan-api-key $(ARBISCAN_API_KEY) -vvvv
else 
	ifeq ($(findstring --network arb_one,$(ARGS)),--network arb_one)
		NETWORK_ARGS := --rpc-url $(ARBITRUM_ONE_RPC_URL) --account $(BURNER) --sender $(BURNER_ADDRESS) --broadcast --verify --etherscan-api-key $(ARBISCAN_API_KEY) -vvvv
	endif
endif
...
#                              ZKSYNC
# //////////////////////////////////////////////////////////////*/
zkbuild:; foundryup-zksync && forge build --zksync --system-mode=true && foundryup
